...
class ListNode:
    ...
    def insert_next(self, node):
        # if this node has a previous node,
        # make sure it does not refer to this node any longer
        if node.prev:
            node.prev.next = None  # cleans up previous node
        # now node.prev can be set to this node, and
...
class DoublyLinkedList:
    ...
    def add_to_head(self, value):
        ...
        # case 2: linked list has 1+ nodes
        node = ListNode(value)
